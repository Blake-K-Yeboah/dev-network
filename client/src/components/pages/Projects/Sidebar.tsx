...
const Sidebar = inject('authStore', 'projectStore', 'usersStore', 'communityStore')(observer(({ authStore, projectStore, usersStore, communityStore }: IStoreProps) => {
    ...
    return (
        <div className={`sidebar ${status ? 'open' : ''}`}>
            ...
            <button className={`btn outline ${hideStatus} ${route !== '/community' ? 'hidden' : ''}`} onClick={(): void => communityStore.toggleStatus()}>Create a Post <FaPlus style={iconStyle} /></button>
